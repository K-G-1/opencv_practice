...
public:
	...
	Image(const string pFilename);
	Image(const cv::Mat &pMat);
	//load a file into obj
	int LoadImg(string pFilename);
	//show the obj m_Mat on window
	void ShowInWindow(const string pWinName);
	//save the obj m_Mat as image file
	void SaveImg(const string pFilepath);
	//RGB2GRAY
	void toGray();
