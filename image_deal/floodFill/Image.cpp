...
int Image::LoadImg(string pFilename)
{
	m_Mat = imread(pFilename);
	// try {
	// 	if(m_Mat.empty())
...
void Image::SelfDilate(int i)
{
	Mat element_dilate;

	element_dilate = getStructuringElement(MORPH_ELLIPSE,Size(i,i));
	dilate(m_Mat,m_Mat,element_dilate);
}
